...
Initital event is executed.
Trigger actions execute.


```sql
CREATE TRIGGER trgAfterInsert --  reate trigger
ON Employees -- Attach trigger to table
...
Initial event fires the trigger.
Initital event is not executed anymore.
Trigger actions execute.

```sql
CREATE TRIGGER trgInsteadOfInsert --  Create trigger
ON Employees -- Attach trigger to table
...
```sql
SELECT OBJECT_DEFINITION(OBJECT_ID('trgAfterInsert')) AS TriggerDefinition;
```
...
```sql
DROP TRIGGER triggerName ON DATABASE; -- databse level triggers
```
...
AS
BEGIN
    INSERT INTO EmployeesAudit (EmployeeID, ChangeType, ChangedBy)
    SELECT i.EmployeeID, 'INSERT', SUSER_SNAME()
    FROM inserted i;
END;
```
...
BEGIN
    ...
    BEGIN
        PRINT 'Salary changed for EmployeeID: ' + CAST(@EmployeeID AS NVARCHAR(10)) +
              ' from ' + CAST(@OldSalary AS NVARCHAR(20)) +
...
BEGIN
    ...
    BEGIN
        PRINT 'Employee with ID ' + CAST(@EmployeeID AS NVARCHAR(10)) + ' has been deleted.';
    END
END;
```
...
```sql
CREATE TRIGGER trgPreventDropDatabase
ON DATABASE
FOR DROP_TABLE
AS
RAISERROR('...')
ROLLBACK; -- Prevent the DROP TABLE operation
```
...
Logon triggers fired after authentication phase (username / password check) but before user session established (when SQL Server info becomes avaimable for queries).
